...
async def create_admin(
    request: Request, user_: New_User, db: Session = Depends(get_db)
):
    ...
    new_admin = User(
        username=user_.username, password=hashed_password.decode("utf-8"), role="admin"
    )
    db.add(new_admin)
    try:
        db.commit()
        ...
        return make_response(
            200, "success", {"user_id": new_admin.id, "username": new_admin.username}
        )
...
async def update_authority(
    request: Request, user_id: int, role_: Role, db: Session = Depends(get_db)
):
    role = role_.role
    ...
    db.commit()
    return make_response(200, "role updated", {"user_id": user_id, "role": role})


@router.get("/users/")
async def user_list(
    request: Request, page=1, page_size=10, db: Session = Depends(get_db)
):
    this_id = request.session.get("user_id")
    if not this_id:
        return make_response(401, "not logged in", None)
