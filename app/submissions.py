...
async def submit_code(
    request: Request, submiss: submission, db: Session = Depends(get_db)
):
...
async def get_submission(
    request: Request, lis: submission_list = Depends(), db: Session = Depends(get_db)
):
    ...
    query = db.query(
        Submission.id, Submission.status, Submission.score, Submission.counts
    )
    ...
    for sub in submissions:
        if sub.status in ("error", "pending"):
            result.append({"submission_id": sub.id, "status": sub.status})
        else:
            result.append(
                {
...
async def rejudge(request: Request, submission_id: int, db: Session = Depends(get_db)):
    ...
    return make_response(
        200, "rejudge started", {"submission_id": sub.id, "status": "pending"}
    )
...
async def get_log(request: Request, submission_id: int, db: Session = Depends(get_db)):
    ...
    if (
        ...
        and sub.problem.public_cases is False
    ):
        log_his.status = 403
        db.add(log_his)
        db.commit()
        db.refresh(log_his)
        return make_response(403, "permission denied", None)
    data = {"details": sub.detail, "score": sub.score, "counts": sub.counts}
    log_his.status = 200
    db.add(log_his)
    db.commit()
    db.refresh(log_his)
