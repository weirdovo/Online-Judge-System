...
@router.post("/")
async def add_language(request : Request, new_lang : new_language, db : Session = Depends(get_db)):
    user_id = request.session.get("user_id")
    if not user_id:
        return make_response(401, "not logged in", None)
    existing_language = db.query(Language).filter(Language.name == new_lang.name).first()
    if existing_language:
        ...
        try:
            ...
            return make_response(200, "language registered", {"name": existing_language.name})
